---
title: "All spheroids comparison: time at buzz onset"
...
```{r}
lm_buzz = lm(time_buzz2touch ~ position + position:SPHEROID, data=df)
summary(lm_buzz)

glm_buzz = glm(time_buzz2touch ~ position + position:SPHEROID, data=df, family=Gamma(link="log"))
summary(glm_buzz)
```
...
We can see that the `glm` model fits a little better than the `lm` model on the quantile variation, so let’s use the `glm` model going forward.
...
```{r}
emm_lm = emmeans(glm_buzz, ~position, type="response")
emm_lm

contrast(emm_lm, type="response")

emm_lm_2level = emmeans(glm_buzz, ~SPHEROID|position, type="response")
emm_lm_2level
