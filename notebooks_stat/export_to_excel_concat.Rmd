...
df_all = bind_rows(spheroid_position_list) %>%
  add_column(
    fig.panel = c(
      "Fig. 9A", "Fig. 9B left", "Fig. 9B right",
      "Fig. 9C", "Fig. 9D", "Fig. 9E", "Fig. 9F"
    ), .before = "contrast"
  ) %>%
...
stat_spheroid_panel = list(
  "A_sph_decision_time" =             "Fig. 9A",
  "B_sph_decision_dist_to_sphere" =   "Fig. 9B left",
  "B_sph_decision_dist_to_spheroid" = "Fig. 9B right",
  "C_sph_buzz_time" =                 "Fig. 9C",
  "D_sph_buzz_range" =                "Fig. 9D",
  "E_sph_scan_sphere" =               "Fig. 9E",
  "F_sph_scan_spheroid" =             "Fig. 9F"
)
```
...
df_all = bind_rows(ar13_position_list) %>%
  add_column(
    fig.panel = c(
      "Fig. 8B", "Fig. 10A", "Fig. 10B left", "Fig. 10B right",
      "Fig. 10C", "Fig. 10D", "Fig. 10E", "Fig. 10F"
    ),
    .before = "contrast"
  ) %>%
  rename("Fig panel" = "fig.panel",
...
ar13_cluster_panel = list(
  "last_scan_duration" =              "Fig. 8B",
  "A_clu_decision_time" =             "Fig. 10A",
  "B_clu_decision_dist_to_sphere" =   "Fig. 10B left",
  "B_clu_decision_dist_to_spheroid" = "Fig. 10B right",
  "C_clu_buzz_time" =                 "Fig. 10C",
  "D_clu_buzz_range" =                "Fig. 10D",
  "E_clu_scan_sphere" =               "Fig. 10E",
  "F_clu_scan_spheroid" =             "Fig. 10F"
)
```
...
### TC vs CT comparisons
```{r}
df = read.csv(file=paste(stat_path, "ARall_S3_scan_position_contrast.csv", sep="/"))

df = rename_estimate_ratio(df) %>%
    add_column("p.annotation" = apply(df["p.value"], MARGIN = 1, FUN = get_p_star)) %>%
  add_column(fig.panel = c("Fig. S3A"), .before = "contrast"
  ) %>%
  rename("Fig panel" = "fig.panel",
         "Contrast" = "contrast",
         "Estimate or ratio" = "estimate.or.ratio",
...
out_file = file.path(stat_path, "cmp_ARall_S3_position.xlsx")
write.xlsx(df, file = out_file)
print(sprintf("Outputs written to %s", out_file))
```

### AR comparisons
```{r}
df = read.csv(file=paste(stat_path, "ARall_S3_scan_spheroid_contrast.csv", sep="/"))
...
out_file = file.path(stat_path, "cmp_ARall_S3_spheroid.xlsx")
write.xlsx(df, file = out_file)
print(sprintf("Outputs written to %s", out_file))
```
